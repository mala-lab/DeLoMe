...
def accuracy(logits, labels, balance=True, ids_per_cls=None):
    if balance:
...
def mean_AP(args,logits, labels, balance=True, ids_per_cls=None):
...
def evaluate_batch(args, model, g, features, labels, mask, label_offset1, label_offset2, balance=True, ids_per_cls=None):
    model.eval()
    with torch.no_grad():
        ...
        if balance:
            return accuracy(logits, labels.cuda(args.gpu), balance=balance, ids_per_cls=ids_per_cls)
        else:
            return accuracy(logits[mask], labels[mask].cuda(args.gpu), balance=balance, ids_per_cls=ids_per_cls)

def evaluate(model, g, features, labels, mask, label_offset1, label_offset2, balance=True, ids_per_cls=None, save_logits_name=None):
    model.eval()
    with torch.no_grad():
        ...
        if balance:
            return accuracy(logits, labels, balance=balance, ids_per_cls=ids_per_cls)
        else:
            return accuracy(logits[mask], labels[mask], balance=balance, ids_per_cls=ids_per_cls)
...
class NodeLevelDataset(incremental_graph_trans_):
    def __init__(self,name='ogbn-arxiv',IL='class',default_split=False,ratio_valid_test=None,args=None):
        ...
        if name[0:4] == 'ogbn':
            ...
            graph, label = data[0], data[0].ndata['label']
        elif name == 'Arxiv-CL':
            data = DglNodePropPredDataset('ogbn-arxiv', '/data/chniu/phase3/ContinualData')
            graph, label = data[0]
        elif name == 'Products-CL':
            data = DglNodePropPredDataset('ogbn-products', '/data/chniu/phase3/ContinualData')
            graph, label = data[0]
        else:

...
class NET(torch.nn.Module):
    ...
    def observe_task_IL(self, args, g, features, labels, t, train_ids, valid_ids, test_ids, ids_per_cls, dataset):
        """
        ...
        if t!=self.current_task:
            ...
            buffer_node_ids = old_ids[sampled_ids].tolist()
            g, __, _ = dataset.get_graph(node_ids=buffer_node_ids)
            self.aux_g.append(g.to(device='cuda:{}'.format(features.get_device())))
            print('Task:', self.current_task, 'sampled graph size:', g.num_nodes(), 'Edges:', g.num_edges())
            labels_sample = labels[sampled_ids]
            if args.cls_balance:
                n_per_cls = [(labels_sample == j).sum() for j in range(args.n_cls)]
                loss_w_ = [1. / max(i, 1) for i in n_per_cls]  # weight to balance the loss of different class
            else:
                loss_w_ = [1. for i in range(args.n_cls)]
            loss_w_ = torch.tensor(loss_w_).to(device='cuda:{}'.format(args.gpu))
